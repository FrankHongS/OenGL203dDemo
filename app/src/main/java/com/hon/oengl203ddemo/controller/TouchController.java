package com.hon.oengl203ddemo.controller;
...
public class TouchController {
    ...
    public TouchController(BaseActivity context,GLSurfaceView view, ModelRenderer renderer) {
        super();
...
//                    if(Math.abs(zoomFactor)>0.01f)
//                        mRenderer.getCamera().MoveCameraZ(Math.signum(length-previousLength)*0.03f);
                }
				if (isRotating) {
					touchStatus = TOUCH_STATUS_ROTATING_CAMERA;
					Log.d("Camera", "Rotating camera '" + Math.signum(rotationVector[2]) + "'..."+rotationVector[2]);
                    Log.d("Camera", "length "+Math.abs(length-previousLength));
                    mRenderer.getCamera().Rotate((float) (Math.signum(rotationVector[2]) / Math.PI) / 4);
				}
            }else if(pointerCount==3){
                Log.d("hon", "pointerCount=3 ");
    ...
    private void selectObjectImpl(float[] nearPoint, float[] farPoint) {
        SceneLoader scene = mActivity.getSceneLoader();
        if (scene == null) {
            return;
        }
